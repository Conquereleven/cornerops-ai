...
const main = async () => {
  ...
  const service = new CommercialOperationsService({ store, config: { corneropsCommercialOperationsEnabled: true, corneropsCommercialDefaultLocalShippingAed: 15 } });
  ...
  await service.createQuote({ quoteId: 'demo-quote-draft', accountId: opportunity.accountId, opportunityId: opportunity.opportunityId, currency: 'AED', shippingRule: 'default_local_uae', lineItems: [{ skuId: 'demo-sku-tajin', quantity: 1, unitPrice: 10, priceSource: 'founder_demo_approved' }] }, context);
  await service.createQuote({ quoteId: 'demo-quote-approved', accountId: opportunity.accountId, opportunityId: opportunity.opportunityId, currency: 'AED', shippingRule: 'default_local_uae', lineItems: [{ skuId: 'demo-sku-pulparindo', quantity: 2, unitPrice: 25, priceSource: 'founder_demo_approved' }] }, context);
  ...
  await service.recordPayment(order.orderId, { method: 'BANK_TRANSFER', amount: order.total, currency: 'AED', status: 'PENDING_VERIFICATION', reference: 'demo-bank-transfer' }, context);
  await store.create('order', 'demo-cod-order', { orderId: 'demo-cod-order', accountId: 'demo-account-1', quoteId: 'demo-quote-approved', lineItems: [], currency: 'AED', total: 12, paymentMethod: 'CASH_ON_DELIVERY', paymentStatus: 'PENDING', fulfillmentStatus: 'READY_TO_PICK', deliveryStatus: 'PENDING', owner: 'founder-demo', status: 'ORDER_CONFIRMED' }, context);
  await store.create('fulfillment', 'demo-delayed-fulfillment', { fulfillmentId: 'demo-delayed-fulfillment', orderId: order.orderId, provider: 'demo_3pl', assignedTo: 'founder-demo', status: 'BLOCKED', blockers: ['COMMERCIAL_DEMO_DATA_NOT_PRODUCTION'] }, context);
  ...
  await store.create('fulfillment', 'demo-failed-delivery', { fulfillmentId: 'demo-failed-delivery', orderId: 'demo-cod-order', provider: 'demo_3pl', assignedTo: 'founder-demo', status: 'DELIVERY_FAILED', blockers: [] }, context);
