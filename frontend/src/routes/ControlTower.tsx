...
import { askControlTower, decideApprovalDryRun, getControlTowerApprovals, getControlTowerV08 } from '../lib/api';
...
export function ControlTower() {
  ...
  return <div className="control-tower-page">
    <header className="ct-title"><div><span className="eyebrow">Internal operator surface · v0.8</span><h1>Control Tower</h1><p>Visibility and policy-routed operation. CornerOps remains the source of truth.</p></div><div className="ct-title-status">{report && <><StatusBadge tone={toneFor(report.status)}>{report.status}</StatusBadge><StatusBadge tone="amber">{report.mode}</StatusBadge><StatusBadge tone="blue">DRY-RUN</StatusBadge></>}<button onClick={() => void load()} disabled={loading}><RefreshCw className={loading ? 'spin' : ''} size={14} />Refresh</button></div></header>
    ...
    {report && <>
      ...
      <ApprovalCenter approvals={approvals.items} busyId={busyApproval} onDecision={(id, decision) => void decide(id, decision)} />
