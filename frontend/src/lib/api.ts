...
export type SellerMediaItem={id:string;sellerId:string;supplierCatalogItemId:string;mediaType:string;managedAssetUrl?:string|null;sourceImageUrl?:string|null;status:string};
...
export const getSupplyGraphMatchRuns=(token='')=>intelligenceRequest<{matchRuns:Array<{id:string;comparisonScope:string;supplierCountEvaluated:number;overallMatchScore:number;overallConfidenceScore:number;createdAt:string}>}>('/api/intelligence/supplygraph/match-runs?limit=25',token);
