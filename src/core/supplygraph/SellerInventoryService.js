  ...
  async initialize({sellerId,catalogItemIds=[],approvalId,actorId='founder'}={}){
    ...
    return this.store.withTransaction(async(client)=>{
      ...
      for(const catalogItemId of [...new Set(catalogItemIds)].sort()){
        const key=`supplygraph-inventory-v1.13:${sellerId}:${catalogItemId}`;
        const ledger=await client.query(`insert into ${this.store.table('seller_inventory_ledger')}(seller_id,supplier_catalog_item_id,movement_type,quantity_delta,unit,source_type,source_reference,idempotency_key,authorization_basis,physical_count_verified,created_by,reason) values($1,$2,'initial_seed',$3,'operational_units','founder_authorized_initialization',$4,$5,'founder_attestation',false,$6,'Founder-authorized operational initialization; not a physical count') on conflict(idempotency_key) do nothing returning *`,[sellerId,catalogItemId,LIMITS.initialProductStock,approvalId,key,actorId]);
