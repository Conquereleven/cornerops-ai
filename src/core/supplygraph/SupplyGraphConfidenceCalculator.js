...
class SupplyGraphConfidenceCalculator {
  calculate(demand, catalog, offer, match, { staleAfterHours = 168, now = Date.now() } = {}) {
    ...
    const resolved = offer?.resolvedEvidence?.fields || {};
    const priceObservedAt = resolved.price?.observedAt || offer?.observedAt;
    const stale = !priceObservedAt || Date.parse(priceObservedAt) < now - staleAfterHours * 3600000;
    const priceFreshness = offer?.unitPrice !== null && offer?.unitPrice !== undefined ? (stale ? 5 : 15) : 0;
    const usable = (name) => !resolved[name]?.conflict && resolved[name]?.known !== false && !resolved[name]?.stale;
    const stock = usable('stock_status') && offer?.stockStatus && offer.stockStatus !== 'unknown' ? (usable('stock_quantity') && offer.stockQuantity !== null && offer.stockQuantity !== undefined ? 20 : 12) : 0;
    const moq = usable('minimum_order') && offer?.minimumOrderQuantity !== null && offer?.minimumOrderQuantity !== undefined ? 10 : 0;
    const leadTime = usable('lead_time_days') && offer?.leadTimeDays !== null && offer?.leadTimeDays !== undefined ? 10 : 0;
    const shelfTemperature = (usable('shelf_life_days') && offer?.shelfLifeDays !== null && offer?.shelfLifeDays !== undefined ? 5 : 0)
      + (usable('temperature_zone') && catalog.temperatureZone ? 5 : (!demand.temperatureZone ? 5 : 0));
